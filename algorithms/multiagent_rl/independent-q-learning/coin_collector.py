    ...
    def action_distribution(self, env: CoinCollectionEnv, n_rollouts: int = 100) -> np.ndarray:
        ...
        for _ in range(n_rollouts):
            ...
            for _ in range(CFG.max_steps):
                ...
                counts[a] += 1
                s, _, _, _ = env.step([a, a])   # crude — just for distribution
        return counts / counts.sum()

    def value_map(self, grid_size: int) -> np.ndarray:
        """
        Return a (grid_size × grid_size) array of mean max-Q values.
...
def train(cfg: Config = CFG, verbose: bool = True) -> Tuple[List[QLearningAgent], TrainingLog]:
    ...
    if verbose:
        ...
        print("═" * 60)
        print(f"  Multiagent RL — Coin Collection Game")
...
def plot_value_heatmaps(agents: List[QLearningAgent], cfg: Config):
    ...
    colors_list = [COLORS["agent_A"], COLORS["agent_B"]]
...
def replay_ascii(
    ...
    print("\n" + "─" * 40)
    print(f"  Greedy Episode Replay ({cfg.grid_size}×{cfg.grid_size})")
    print("─" * 40)
...
def main():
    ...
    """
    # ── quick-run flag ───────────────────────────────────────────
    # Set QUICK=True to train fewer episodes for rapid testing
    ...
    # Training dashboard — competitive
    ...
    # Training dashboard — cooperative
    ...
    fig6, anim = animate_episode(
        ...
        steps=50,
        interval_ms=300,
        save_path="marl_episode.gif",   # remove or set None to skip saving
    )
